...
const deleteMedia = (index) => {
    confirm.require({
        message: 'Hapus media ini dari galeri produk?',
        header: 'Hapus Media',
                    ...
                    <div class="space-y-6 pt-4 border-t border-gray-100 dark:border-gray-800">
                        <div class="flex items-center justify-between pb-1.5 border-b border-gray-100 dark:border-gray-800">
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 mt-0.5">Kelola foto utama, foto galeri tersimpan, dan foto galeri tambahan.</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div v-if="!mainImagePreview">
                                ...
                                >
                                    ...
                                    <p class="text-xs font-bold text-gray-700 dark:text-gray-200">
                                        Klik atau seret foto ke sini untuk mengganti <span class="text-amber-600 dark:text-amber-400">Gambar Utama</span>
                                    </p>
                            ...
                            <div v-else class="flex flex-wrap items-center gap-4">
                                ...
                                <div class="space-y-1">
                                    <p class="text-xs font-bold text-gray-800 dark:text-gray-200">Foto Sampul Terpasang</p>
                                    <p class="text-[10px] text-gray-400">Gambar ini merupakan tampilan utama produk di katalog.</p>
                                    <Button label="Ganti Sampul" icon="pi pi-refresh" severity="secondary" size="small" outlined @click="triggerMainInput" />
                                </div>
                            </div>
                        </div>

                        <!-- Existing Saved Gallery -->
                        <div v-if="product.media_assets && product.media_assets.length > 0" class="space-y-2 pt-2">
                            <div class="flex items-center justify-between">
                                <label class="text-xs font-semibold">Galeri & Media Tersimpan</label>
                                <span class="text-[10px] text-gray-400 font-medium">
                                    {{ product.media_assets.length }} media tersimpan di server
                                </span>
                            </div>
                            <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
                                <div
                                    ...
                                    :key="idx"
                                    class="relative group aspect-square bg-gray-900 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm"
                                >
                                    <img v-if="media.type === 'image'" :src="`/${media.url}`" class="w-full h-full object-cover" alt="Saved gallery" />
                                    <div v-else class="w-full h-full flex flex-col items-center justify-center p-2 text-center text-amber-400 bg-gray-900">
                                        <i class="pi pi-video text-2xl mb-1"></i>
                                        <span class="text-[9px] max-w-full truncate font-medium text-gray-300">{{ media.url }}</span>
                                    </div>
                                    
                                    <span class="absolute top-1.5 left-1.5 bg-emerald-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                                        Tersimpan
                                    </span>
                                    ...
                                    <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                                        ...
                                        <button
                                            ...
                                            class="p-1.5 bg-red-600 hover:bg-red-500 text-white rounded-lg transition shadow"
                        ...
                        <div class="space-y-2 pt-2">
                            <div class="flex items-center justify-between">
                                ...
                                <span v-if="galleryPreviews.length > 0" class="text-[10px] text-amber-600 dark:text-amber-400 font-bold">
                                    {{ galleryPreviews.length }} foto baru akan diunggah
                                </span>
                            </div>
                            ...
                            <!-- New Gallery Grid + Add Box -->
                            <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
                                <!-- Newly Selected Previews -->
                                <div
                                    v-for="(item, idx) in galleryPreviews"
                                    :key="idx"
                                    class="relative group aspect-square rounded-xl overflow-hidden border border-amber-400 dark:border-amber-500/60 shadow-sm bg-gray-900"
                                >
                                    <img :src="item.url" class="w-full h-full object-cover" alt="New gallery preview" />
                                    <span class="absolute top-1.5 left-1.5 bg-amber-500 text-gray-950 text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                                        Baru #{{ idx + 1 }}
                                    </span>
                                    <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
                                        <button
