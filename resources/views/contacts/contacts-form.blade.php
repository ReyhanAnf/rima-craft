...
<div class="px-4 sm:px-4 py-6 pb-24">
    <form hx-{{ $contact->exists ? 'put' : 'post' }}="{{ $contact->exists ? route('contacts.update', $contact) : route('contacts.store') }}" hx-target="#contacts-list" hx-swap="innerHTML" class="space-y-6" x-data="{ submitting: false }" @submit="submitting = true" @htmx:after-request="submitting = false">
        ...
        <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tipe Kontak <span class="text-red-500">*</span></label>
            <select name="type" required class="w-full px-4 py-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition hover:border-gray-300 dark:hover:border-gray-600">
        ...
        <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nama Kontak <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="{{ $contact->name }}" required class="w-full px-4 py-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition hover:border-gray-300 dark:hover:border-gray-600">
        </div>
        <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nomor Telepon</label>
            <input type="text" name="phone" value="{{ $contact->phone }}" class="w-full px-4 py-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition hover:border-gray-300 dark:hover:border-gray-600">
        </div>
        <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Alamat</label>
            <textarea name="address" rows="3" class="w-full px-4 py-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition hover:border-gray-300 dark:hover:border-gray-600">{{ $contact->address }}</textarea>
        </div>
        <div class="fixed bottom-0 right-0 w-full max-w-md bg-white dark:bg-gray-950 p-4 border-t border-gray-200 dark:border-gray-800 z-20">
            <button type="submit" class="w-full flex justify-center items-center gap-2 py-3 px-4 rounded-md text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed" x-bind:disabled="submitting">
